...
アプリをビルドするためにエミュレーション環境の「QEMU」を利用するワークフローの事例については、[GitHub Action 設定ガイド](/ja/distribute/pipelines/github/#arm-ランナーのコンパイル) の章をご覧ください。ただし、この処理は非常に遅いため、Build Minutes（ビルド時間）が無料のパブリック・リポジトリでのみ推奨されます。プライベート・リポジトリでは、GitHub の「ARM ランナー」（上記リンク）の方がコスト効率が良く、セットアップもはるかに簡単です。
