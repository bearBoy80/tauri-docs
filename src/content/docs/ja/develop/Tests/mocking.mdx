...
`shouldMockEvents` オプションを使用すると、[Event System] の部分的なサポートにより、Rust コードから発行されるイベントをシミュレートできます：
...
`emitTo` と `emit_filter` はまだサポートされて**いません**。
...
[Event System]: /ja/develop/calling-frontend/#event-system
