...
앱을 빌드하기 위해 에뮬레이션 환경인 "QEMU"를 이용하는 워크플로 사례에 대해서는 [GitHub Action 설정 가이드](/ko/distribute/pipelines/github/#arm-러너-컴파일) 장을 참조하십시오. 단, 이 처리는 매우 느리므로 Build Minutes(빌드 시간)가 무료인 공개 리포지토리에서만 권장됩니다. 비공개 리포지토리에서는 GitHub의 "ARM 러너"(위 링크)가 비용 효율적이며 설정도 훨씬 간단합니다.
